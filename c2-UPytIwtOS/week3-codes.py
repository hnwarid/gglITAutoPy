...
# followed by a phone number field, and a role field. The phone number field contains
# U.S. phone numbers, and needs to be modified to the international format, with "+1-"
# in front of the phone number. Fill in the regular expression, using groups,
# to use the transform_record function to do that.
def transform_record(record):
    # new_record = re.sub(r"([A-Za-z] [A-Za-z]),([\d\-]{3}),([\w ])", r"\1, +1-\2, \3", record)
    # doesn't work and complicated...
    new_record = re.sub(r",([\d-]{3})", r",+1-\1", record)  # this works
    return new_record
...
def multi_vowel_words(text):
    pattern = r"[A-Za-z]*[AIUEOaiueo]{3,}[A-Za-z]*"
...
# The transform_comments function converts comments in a Python script into those usable by a
# C compiler. This means looking for text that begins with a hash mark (#) and replacing it
...
def transform_comments(line_of_code):
    pattern = r"[#]{1,} "
    result = re.sub(pattern, r"// ", line_of_code)
    return result
